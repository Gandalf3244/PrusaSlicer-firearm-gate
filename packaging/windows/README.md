...
(the refused part is generated by `make_test_part.py`: a block drilled with one
fingerprint's hole pattern - not a gun part).
...
firearm models; `python packaging\windows\make_test_part.py firearm-check x.stl` makes a part
